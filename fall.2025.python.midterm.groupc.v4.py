...
for col in X_train.select_dtypes(include=[np.number]).columns[:3]:
    plt.figure()
    counts, bins, patches = plt.hist(X_train[col].dropna(), bins=30, edgecolor='black')
...
if "FinalPrice" in df.columns:
    ...
    counts, bins, patches = plt.hist(df["FinalPrice"], bins=30, color='skyblue', edgecolor='black')
    plt.title("Distribution of Final Price", fontsize=14, fontweight='bold')
...
if "Rating" in df.columns:
    plt.figure(figsize=(8, 5))
    ratings_clean = df["Rating"].dropna().astype(float)
    counts, bins, patches = plt.hist(ratings_clean, bins=[1,2,3,4,5,6], color='lightgreen', edgecolor='black', align='left', rwidth=0.8)
...
if len(num_cols) > 0:
    ...
    plt.text(1.05, median_val, f'Median: {median_val:.2f}', va='center', fontsize=9)

    plt.tight_layout()
    plt.show()

# Correlation heatmap (labels already present)
if len(num_cols) > 1:
...
# Bar plot with count labels for Brand
...
# Bar plot with count labels for Region
...
if "OrderDate" not in df_ts.columns:
    ...
    if not g_feat.empty:
        ...
        if not X_te.empty:
            ...
            # Enhanced line chart for one group: actual vs. predicted demand
            if not forecast_out.empty:
                ...
                if group_cols:
                    ...
                    plt.plot(subset["YearMonth"], subset["ActualUnits"], marker='o', label="Actual", linewidth=2)
                    plt.plot(subset["YearMonth"], subset["PredUnits"], marker='s', label="Forecast", linewidth=2)

                    # Add value labels
                    for x, y in zip(subset["YearMonth"], subset["ActualUnits"]):
                        plt.text(x, y + 0.5, f'{y:.0f}', ha='center', fontsize=8)
                    for x, y in zip(subset["YearMonth"], subset["PredUnits"]):
                        plt.text(x, y - 0.5, f'{y:.0f}', ha='center', fontsize=8, color='gray')

                    plt.title(f"Units Forecast – {dict(zip(group_cols, grp_example))}", fontsize=14, fontweight='bold')
...
if has_rating_col:
    ...
    if importances is not None and len(importances) == len(feat_names):
        ...
        plt.figure(figsize=(10, 6))
        bars = plt.bar(imp_df["feature"], imp_df["importance"], color="#4a90e2", edgecolor="black")
        ...
        plt.title("Top Feature Importances – InventoryAction", fontsize=14, fontweight="bold")
